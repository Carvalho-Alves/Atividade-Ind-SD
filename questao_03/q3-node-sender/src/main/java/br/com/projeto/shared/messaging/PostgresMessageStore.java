...
public class PostgresMessageStore implements MessageStore {
  ...
  public void nack(UUID messageId, String receiverId, String error) {
    ...
    Integer attemptCount = jdbc.queryForObject(
        "SELECT attempt_count FROM messages WHERE id = :id",
        new MapSqlParameterSource().addValue("id", messageId),
        Integer.class
    );

    int safeAttemptCount = (attemptCount != null) ? attemptCount : 1;
    long capSeconds = Math.max(1L, properties.getLease().getLockDuration().toSeconds());
    long expMultiplier = 1L << Math.min(5, Math.max(0, safeAttemptCount - 1));
    long delaySeconds = Math.min(capSeconds, 5L * expMultiplier);
    Instant nextAttemptAt = now.plus(Duration.ofSeconds(delaySeconds));
    Timestamp nextAttemptAtTs = Timestamp.from(nextAttemptAt);

    String sql = """
        UPDATE messages
      SET status = 'LEASED',
            lock_owner = NULL,
        locked_until = :nextAttemptAt,
            last_error = :error,
            updated_at = :now
        WHERE id = :id
          AND status = 'LEASED'
    ...
    jdbc.update(sql, new MapSqlParameterSource()
      ...
      .addValue("nextAttemptAt", nextAttemptAtTs, Types.TIMESTAMP)
        .addValue("now", nowTs, Types.TIMESTAMP));
